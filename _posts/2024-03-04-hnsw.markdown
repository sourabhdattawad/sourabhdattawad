---
layout: post
title:  "Developing HNSW (Hierarchical Navigable Small World) from scratch to perform approximate nearest neighbor search"
...
This extends the concept of NSW (Navigable Small World) by introducing multiple layers stacked atop one another. Each node may exist on a subset or all of the defined layers. The determination of a node's occurrence on a layer is computed using the formula:
